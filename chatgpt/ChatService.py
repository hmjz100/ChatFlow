...
class ChatService:
	...
	async def set_dynamic_data(self, data):
		if self.req_token:
			...
			log.info("Request token is empty, use no-auth 3.5")
		...
		log.info(f"Request token: {self.req_token}")
		log.info(f"Request proxy: {self.proxy_url}")
		log.info(f"Request UA: {self.user_agent}")
		log.info(f"Request impersonate: {self.impersonate}")
	...
	async def set_model(self):
		...
		# 查找并设置 req_model
		self.req_model = next((v for k, v in model_map.items() if k in self.origin_model), "gpt-4o")
			...
			if r.status_code == 200:
				...
				if self.persona != "chatgpt-paid":
					if self.req_model == "gpt-4" or self.req_model == "o1-preview":
						log.error(f"Model {self.resp_model} not support for {self.persona}")
						raise HTTPException(
							status_code=404,
							detail={
	...
	async def prepare_send_conversation(self):
		try:
			...
			log.error(f"Failed to format messages: {str(e)}")
		...
		if self.gizmo_id:
			...
			log.info(f"Gizmo id: {self.gizmo_id}")
		else:
			conversation_mode = {"kind": "primary_assistant"}

		log.info(f"Model mapping: {self.origin_model} -> {self.req_model}")
		self.chat_request = {
			"action": "next",
			...
			if r.status_code != 200:
				...
				if "application/json" == r.headers.get("Content-Type", ""):
					...
					if "cf_chl_opt" in rtext:
						# log.error(f"Failed to send conversation: cf_chl_opt")
						raise HTTPException(status_code=r.status_code, detail="cf_chl_opt")
					if r.status_code == 429:
						# log.error(f"Failed to send conversation: rate-limit")
						raise HTTPException(status_code=r.status_code, detail="rate-limit")
					detail = r.text[:100]
				# log.error(f"Failed to send conversation: {detail}")
				raise HTTPException(status_code=r.status_code, detail=detail)
	...
	async def get_download_url_from_upload(self, file_id):
		...
		try:
			...
			log.error(f"Failed to get download url from upload: {e}")
			return ""
	...
	async def get_upload_url(self, file_name, file_size, use_case="multimodal"):
		...
		try:
			...
			if r.status_code == 200:
				...
				log.info(f"file_id: {file_id}, upload_url: {upload_url}")
				return file_id, upload_url
			else:
				raise HTTPException(status_code=r.status_code, detail=r.text)
		except Exception as e:
			log.error(f"Failed to get upload url: {e}")
			return "", ""
	...
	async def upload_file(self, file_content, mime_type):
		...
		if mime_type.startswith("image/"):
			try:
				...
				log.error(f"Error image mime_type, change to text/plain: {e}")
		...
		if file_id and upload_url:
			if await self.upload(upload_url, file_content, mime_type):
				...
				if download_url:
					...
					log.info(f"File_meta: {file_meta}")
					return file_meta
	...
	async def get_response_file_url(self, conversation_id, message_id, sandbox_path):
		try:
			...
			log.info("Failed to get response file url")
			return None
