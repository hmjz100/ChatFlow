...
async def format_not_stream_response(response, prompt_tokens, max_tokens, model):
	...
	async for chunk in response:
		try:
			...
			log.error(f"Error: {chunk}, error: {str(e)}")
			continue
...
async def wss_stream_response(websocket, conversation_id):
	while not websocket.closed:
		try:
			...
			log.error("Timeout! No message received within the specified time.")
			break
		except websockets.ConnectionClosed as e:
			if e.code == 1000:
				log.error("WebSocket closed normally with code 1000 (OK)")
				yield b"data: [DONE]\n\n"
			else:
				log.error(f"WebSocket closed with error code {e.code}")
		except Exception as e:
...
async def stream_response(service, response, model, max_tokens):
	...
	async for chunk in response:
		chunk = chunk.decode("utf-8")
		if end:
			log.info(f"Response Model: {model_slug}")
			yield "data: [DONE]\n\n"
			break
		try:
			if chunk.startswith("data: {"):
					...
					if content.get("content_type") == "multimodal_text":
						...
						for part in parts:
							...
							if inner_content_type == "image_asset_pointer":
								...
								log.debug(f"image_download_url: {image_download_url}")
			...
			if chunk.startswith("data: "):
				...
				if chunk_data.get("error"):
					log.error(f"Error: {chunk_data.get('error')}")
					yield "data: [DONE]\n\n"
					break
			log.error(f"Error: {chunk}, details: {str(e)}")
			continue
...
def format_messages_with_url(content):
	...
	while True:
		...
		if url:
			url_list.append(url)
			log.info(f"Found a file_url from messages: {url}")
		else:
			break
	if not url_list:
		return content
